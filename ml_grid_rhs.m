function dxdt = ml_rhs(t,x,theta,gamma_,Dr,Dd,K,M)
...
for i = 2:K-1
    ...
    for j = 2:M-1
        L(i,j) = Dd(i,j)*V(i+1,j) ...
            ...
            + Dr(i,j)*V(i,j+1) ...
            + Dr(i,j-1)*V(i,j-1) ...
            - (Dd(i,j)+Dd(i-1,j)+Dr(i,j)+Dr(i,j-1))*V(i,j);
    end
    % right edge
    L(i,M) = Dd(i,M)*V(i+1,M) ...
        + Dd(i-1,M)*V(i-1,M) ...
        + Dr(i,M-1)*V(i,M-1) ...
        - (Dd(i,M)+Dd(i-1,M)+Dr(i,M-1))*V(i,M);
end
% lower left corner
...
for j = 2:M-1
    L(K,j) = Dd(K-1,j)*V(K-1,j) ...
        + Dr(K,j)*V(K,j+1) ...
        + Dr(K,j-1)*V(K,j-1) ...
        - (Dd(K-1,j)+Dr(K,j)+Dr(K,j-1))*V(K,j);
end
% lower right corner
L(K,M) = Dd(K-1,M)*V(K-1,M) ...
    + Dr(K,M-1)*V(K,M-1) ...
    - (Dd(K-1,M)+Dr(K,M-1))*V(K,M);
